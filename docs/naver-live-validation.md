...
> **상태: 검증 완료 (2026-08-25).** 아래 절차대로 실제 블로그에 글 하나를 발행해 전 구간을
> 확인했다. 그때 드러난 실제 구조와 초기 가설의 차이는 README 의 "라이브 검증 상태" 와
> `lib/naver/selectors.ts` 주석에 적혀 있다.
>
> 재검증은 `npm run e2e:editor` 로 한다 — claude CLI 를 부르지 않아 비용이 들지 않고,
> 합성 초안으로 에디터 조작 전 구간을 돌린다. `E2E_PUBLISH=1` 을 주면 발행까지 한다.
